...
class EasyDiffusionAPI:
    ...
    def register_model(self, model_id: str, model_path: Optional[str] = None, skip_download: bool = False) -> bool:
        """
        Register a model with the API
        
        Args:
            model_id: A short identifier for the model (e.g. "sd-v1-5")
            model_path: The path to the model (local or on Hugging Face)
            skip_download: If True, don't attempt to download the model, just register it
        
        Returns:
            bool: True if the model was registered successfully
        """
        ...
        # Register the model
        self.models[model_id] = DiffusionModel(model_id=model_id, model_path=model_path)
        ...
        # If model is on Hugging Face and not skipping download, download it
        if '/' in model_path and not skip_download:
            try:
                if 'sdxl' in model_path.lower() or 'xl' in model_id.lower():
                    # Use the SDXL downloader
                    try:
                        from download_sdxl import download_and_convert_model
                        download_and_convert_model(model_path, model_id)
                    except Exception as e:
                        print(f"Error using SDXL downloader: {e}")
                        return False
                else:
                    # Use the standard SD downloader
                    try:
                        from download_model import download_model
                        download_model(model_path)
                    except Exception as e:
                        print(f"Error using standard downloader: {e}")
                        return False
            except Exception as e:
